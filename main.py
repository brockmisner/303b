...
Price hierarchy:
  1. Chainlink RTDS (wss://ws-live-data.polymarket.com)  — PRIMARY (ground truth)
  2. Binance Futures (wss://fstream.binance.com)          — Lead indicator + sigma
  3. Coinbase WS (wss://ws-feed.exchange.coinbase.com)    — Flow confirmation
...
from oracle_predictor import RTDSPredictor
from oracle_gamma import OracleGammaEngine, clamp
from vacuum_detector import VacuumDetector, VacuumConfig
from synthetic_exit import SyntheticExitEngine, SyntheticExitConfig
from spine import DecisionSpine, Features, Decision
from execution_fsm import ExecutionFSM, ExecFSMConfig
from edge_attribution import EdgeAttribution, AttributionConfig
...
from calibration import load_calibration_overrides, get_bucket_adjustment, bucket_key, t_bucket, spread_bucket, lag_bucket, sigma_bucket
...
from unwind_controller import UnwindControllerV2, UnwindConfig
...
# Entry window guard (seconds remaining)
WINDOW_LO = 20.0
WINDOW_HI = 240.0
...
CALIBRATION_OUTPUT_FILE = os.getenv("CALIBRATION_OUTPUT_FILE", "config/calibration_output.json")
_CAL_OVERRIDES: dict = load_calibration_overrides(CALIBRATION_OUTPUT_FILE)
MASTER_CALIBRATION_FILE = os.getenv("MASTER_CALIBRATION_FILE", "config/master_calibration.json")
CALIBRATION_MAP: dict = {}
try:
    with open(MASTER_CALIBRATION_FILE, "r") as f:
        CALIBRATION_MAP = json.load(f)
        logger.info(f"Loaded {len(CALIBRATION_MAP)} master calibration buckets.")
except Exception as e:
    logger.warning(f"Could not load master calibration map: {e}")
...
RISK_A_BASE = float(os.getenv("RISK_A_BASE", "0.45"))
RISK_B_BASE = float(os.getenv("RISK_B_BASE", "0.60"))
...
def record_pnl(proceeds: float, cost: float) -> None:
    ...
    if CONSEC_LOSSES >= 3 and LOSS_STREAK_TRADES_LEFT == 0:
        ...
        CONSEC_LOSSES = 0  # reset so circuit breaker doesn't fire on same tick
...
class MarketState:
    ...
    btc_ts_ms:     int   = 0
    btc_ts_exch_ms: int  = 0      # Binance exchange timestamp (for sequencing/debug)
    cb_price:      float = np.nan    # Coinbase spot (secondary flow feed)
    cb_ts_ms:      int   = 0        # Coinbase local recv timestamp
...
FLOW         = FlowTracker()
ORACLE_LEAD  = RTDSPredictor()
ORACLE_GAMMA = OracleGammaEngine()
VACUUM_UP    = VacuumDetector(VacuumConfig())
VACUUM_DOWN  = VacuumDetector(VacuumConfig())
SYN_EXIT     = SyntheticExitEngine(SyntheticExitConfig())
...
UNWIND_CTRLS = {
    "UP": UnwindControllerV2(UnwindConfig()),
    "DOWN": UnwindControllerV2(UnwindConfig()),
}
GAMMA         = GammaEngine()
SPINE_UP      = DecisionSpine(gamma_engine=GAMMA, vacuum_detector=VACUUM_UP, synthetic_exit=SYN_EXIT)
SPINE_DN      = DecisionSpine(gamma_engine=GAMMA, vacuum_detector=VACUUM_DOWN, synthetic_exit=SYN_EXIT)
EXEC_FSM      = ExecutionFSM(ExecFSMConfig())
ATTR          = EdgeAttribution(AttributionConfig(log_dir="logs", min_tick_interval_ms=100))
...
LAST_BTC_DIR:     int   = 0
_FLOW_EMA:        float = 0.0
QUOTE_FIRST_SEEN:     dict = {}
PRICE_UPDATE_TIMES:   dict = {}
MAX_PRICE_UPDATE_TRACK: int = 50
FLOW_REVERSAL_LAST_TS:    int = 0
FLOW_REVERSAL_COOLDOWN_MS: int = 300
...
def find_fill_match(token_id: str, price: float, now_ms: int, window_ms: int = 1200, price_tol: float = 0.02, side: str = None, expected_size: float = 0.0) -> Optional[dict]:
    ...
    for f in reversed(RECENT_FILLS):
        if f["ts_ms"] < cutoff: continue  # skip old fills but don't break (timestamps may be non-monotone)
...
def get_master_calib_penalty(side: str, sec_remaining: float, spread: float,
                              lag_p50_ms: float, sigma_ratio: float) -> float:
    """Look up historical win rate from master calibration map and return edge penalty/bonus.
    Low win rate buckets get +0.025 penalty (raise edge bar).
    High win rate buckets get -0.010 subsidy (lower edge bar).
    Returns 0.0 if no data or insufficient fills."""
    if not CALIBRATION_MAP:
        return 0.0
    key = bucket_key(side, t_bucket(sec_remaining), spread_bucket(spread),
                     lag_bucket(lag_p50_ms), sigma_bucket(sigma_ratio))
    entry = CALIBRATION_MAP.get(key)
    if not entry or entry.get("fills", 0) < 10:
        return 0.0
    wr = entry.get("wr", 0.5)
    if wr < 0.45:
        return 0.025   # penalty: raise edge target
    elif wr > 0.65:
        return -0.010  # subsidy: lower edge target
    return 0.0
...
def state_calibration_adjustments(*, side: str, sec_remaining: float, spread: float, lag_p50_ms: float, sigma_ratio: float) -> dict:
    ...
    # Master calibration: EMA-blended historical win rate penalty/bonus
    _mc_penalty = get_master_calib_penalty(side, sec_remaining, spread, lag_p50_ms, sigma_ratio)
    edge_shift += _mc_penalty
...
def get_portfolio_pos() -> PortfolioPos:
    ...
    dom_tid = UP_TOKEN_ID if dom_side == "UP" else (DOWN_TOKEN_ID if dom_side == "DOWN" else None)
...
class DualSigmaEstimator:
    ...
    def _rv(self, window_s: float) -> float:
        ...
        # Normalize by actual time span to handle uneven sample spacing
        t_span_min = (ts_win[-1] - ts_win[0]) / 60000.0
        if t_span_min < 0.05: return 0.0  # < 3s of data
        return float(np.sum(returns ** 2)) / t_span_min
    def compute_sigma(self):
        ...
        sigma_slow = math.sqrt(max(0.0, rv_slow))  # _rv already returns per-minute RV
        sigma_fast = math.sqrt(max(0.0, rv_fast))
...
IN_FLIGHT_QTY: dict     = {"UP": 0.0, "DOWN": 0.0}
IN_FLIGHT_TS: dict      = {"UP": 0, "DOWN": 0}
_IN_FLIGHT_TTL_MS       = 5000

def _release_in_flight(side: str, qty: float):
    IN_FLIGHT_QTY[side] = max(0.0, IN_FLIGHT_QTY[side] - qty)

def _check_in_flight_ttl():
    now = ms_now()
    for s in ("UP", "DOWN"):
        if IN_FLIGHT_QTY[s] > 0 and now - IN_FLIGHT_TS.get(s, 0) > _IN_FLIGHT_TTL_MS:
            IN_FLIGHT_QTY[s] = 0.0
...
def _execute_fok_aggressive(token_id: str, order_side: str, target_size: float, fair_value: float = 0.50, max_slip: float = 0.06, signal_ask: float = 0.0, signal_bid: float = 0.0, is_exit: bool = False) -> Dict[str, Any]:
    ...
    if is_buy:
        ...
        # FIX: Never block exits locally based on size. Let the API decide.
        # If it's a remnant balance close, Polymarket's matching engine will accept it.
        pass
...
async def process_order_task(payload: dict) -> None:
    ...
    _fsm_intent = str(payload.get("intent_id", ""))
                        ...
                        try:
                            ...
                            for _o in (_open_orders or []):
                                if _o.get("asset_id") == token_id or _o.get("token_id") == token_id:
                                    # FIX: Polymarket's get_orders() uses "id", not "orderID"
                                    _oid = _o.get("id") or _o.get("orderID")
                                    if _oid:
                                        logger.info(f"EMERGENCY_CLEAR: Canceling resting order {_oid} for {token_id[:6]}...")
                                        await loop.run_in_executor(thread_pool, client.cancel, _oid)
                            await asyncio.sleep(0.4)  # Give the exchange time to un-reserve the balance
                        except Exception as _ce:
                            logger.error(f"GTC_CLEAR_ERROR: {_ce}")
...
# Allow 100% exact clearance (removed EXIT_BAL_EPS to clear fractions)
                    _available = max(0.0, _exch_inv - _reserved)

                    _sis_frac = float(payload.get("sis_frac", 1.0) or 1.0)
                    
                    # FIX 1: All-or-nothing on remnants. No scaling out if < 5 shares.
                    if _available < 5.0:
                        _sis_frac = 1.0
                        
                    _desired = _available * _sis_frac
                    
                    # FIX 2: Polymarket supports 6 decimals. Use 5 to be safe, eliminating 0.0001 remnants.
                    size = math.floor(min(size, _desired, _local_inv, _available) * 100000) / 100000.0

                    # FIX: Enforce CLOB $5 minimum notional for SELL exits
                    _sell_notional = size * limit_price
                    if _sell_notional < 5.0 and _sell_notional > 0:
                        _notional_min_size = math.ceil(5.0 / max(0.01, limit_price))
                        if _local_inv >= _notional_min_size and _available >= _notional_min_size:
                            size = math.floor(min(_notional_min_size, _local_inv, _available) * 100000) / 100000.0
                        # else: let through as-is for remnant close (PM allows sub-min for position closure)

                    # FIX 3: Instantly write off micro-dust (< 0.05 shares) so we don't infinite loop.
                    if _local_inv > 0 and _local_inv < 1.0 and _available < 0.05:
                        logger.info(f"DUST_SETTLE: Writing off {_local_inv:.6f} shares of micro-dust.")
                        if _pos_local:
                            _pos_local.apply_fill(side=Side.SELL, qty=_local_inv, price=limit_price, ts_ms=ms_now(), fee_per_share=0.0, meta={"path": "DUST_SETTLE"})
                        EXIT_COOLDOWN[cd_key] = ms_now() + 500
                        return

                    MIN_EXIT_SHARES = 5.0

                    # End-of-window dust settle
                    if _local_inv < 1.0 and _available < MIN_EXIT_SHARES and float(STATE.sec_remaining) < 5.0:
                        if _pos_local and _pos_local.inventory > 0:
                    ...
                    # Standard API Minimum routing
                    if size < MIN_EXIT_SHARES:
                        if _available >= MIN_EXIT_SHARES: 
                            size = math.floor(min(MIN_EXIT_SHARES, _available, _local_inv) * 100000) / 100000.0
                        elif _reserved > 0.01:
                            # We have < 5, but some are trapped in pending GTCs. Wait for cancel.
                            EXIT_COOLDOWN[cd_key] = ms_now() + 2000
                            return
                        else:
                            pass # Flows to router for final liquidation!
                ...
                if _fsm_intent:
                    EXEC_FSM.on_result(
                        side=side, intent_id=_fsm_intent,
                        now_ms=ms_now(), state=trade_state,
                        order_id=str(fok_result.get("order_id", "")),
                        filled_qty=float(fok_result.get("used_size", 0)) if ok else 0.0,
                        avg_price=float(fok_result.get("used_limit", 0)) if ok else 0.0,
                    )

                ATTR.log_exec(
                    ts=ms_now(),
                    intent_id=_fsm_intent or str(payload.get("fire_id", "")),
                    side=side, action=order_side, mode=mode,
                    price=limit_price, size_req=size, state=trade_state,
                    order_id=str(fok_result.get("order_id", "")),
                    filled_qty=float(fok_result.get("used_size", 0)) if ok else 0.0,
                    avg_price=float(fok_result.get("used_limit", 0)) if ok else 0.0,
                    realized_edge=0.0, realized_pnl=0.0,
                    detail=str(fok_result.get("error", "")),
                    tags={
                        "spread_tier": str(payload.get("spread_tier", "")),
                        "regime": str(payload.get("regime", "")),
                        "posture": str(payload.get("oracle_posture", "")),
                    },
                )
                ...
                if not ok:
                    ...
                    if ("not enough balance" in err_text or "allowance" in err_text) and order_side == "SELL":
                        ...
                        EXIT_COOLDOWN[cd_key] = ms_now() + 500
                        if order_side == "SELL": _release_in_flight(side, size)
                    elif _is_exit:
                        EXIT_COOLDOWN[cd_key] = ms_now() + 250
                        if order_side == "SELL": _release_in_flight(side, size)
                ...
                if ok:
                    ...
                    if _fill_pos is not None:
                        ...
                        if order_side == "SELL":
                            ...
                            if _actual_qty > 0:
                                ...
                                _release_in_flight(side, _actual_qty)
                        else:
                    ...
                    # NOTE: exec_unlock is handled in the finally block — don't double-unlock
...
def _process_book_update(msg: dict) -> bool:
    ...
    try:
        ...
        _old_bid, _old_ask = state["bid"], state["ask"]
        ...
        if updated and tid in (UP_TOKEN_ID, DOWN_TOKEN_ID):
            ...
            # ── DEFENSIVE: Quote Lifetime & Replacement Churn ──
            if (state["bid"] != _old_bid and state["bid"] > 0) or (state["ask"] != _old_ask and state["ask"] < 1.0):
                if tid not in PRICE_UPDATE_TIMES:
                    PRICE_UPDATE_TIMES[tid] = deque(maxlen=MAX_PRICE_UPDATE_TRACK)
                PRICE_UPDATE_TIMES[tid].append(now)

            # Track quote lifetime (BID)
            _bid_key = (tid, state["bid"], state["bid_size"])
            if state["bid"] > 0 and state["bid_size"] > 0:
                if _bid_key not in QUOTE_FIRST_SEEN:
                    QUOTE_FIRST_SEEN[_bid_key] = now

            # Track quote lifetime (ASK)
            _ask_key = (tid, state["ask"], state["ask_size"])
            if state["ask"] < 1.0 and state["ask_size"] > 0:
                if _ask_key not in QUOTE_FIRST_SEEN:
                    QUOTE_FIRST_SEEN[_ask_key] = now

            # Lazy purge old entries (5% chance per tick)
            if random.random() < 0.05:
                _cutoff = now - 5000
                for k in list(QUOTE_FIRST_SEEN.keys()):
                    if QUOTE_FIRST_SEEN[k] < _cutoff:
                        del QUOTE_FIRST_SEEN[k]
...
async def binance_futures_task() -> None:
    """Binance USDT-M Futures aggTrade — primary BTC price oracle for lead indicator."""
    ...
    while True:
        try:
            async with websockets.connect(url, ping_interval=30, ping_timeout=30) as ws:
                logger.info("Binance Futures WS connected (Primary Lead).")
                retry_delay = 1
                while True:
                    data = json.loads(await ws.recv())

                    price = float(data["p"])
                    ts_exch_ms = int(data.get("E", 0))
                    ts_local_ms = ms_now()

                    STATE.btc_price = price
                    STATE.btc_ts_ms = ts_local_ms          # local clock for latency harness
                    STATE.btc_ts_exch_ms = ts_exch_ms      # exchange clock for sequencing/debug

                    # sigma / tail risk as before
                    _update_sigma(price, ts_local_ms)
                    TAIL_RISK.feed_price(price, ts_local_ms)
                    TAIL_RISK.feed_sigma(STATE.sigma_1m)

                    if not np.isnan(LAST_BTC_PRICE) and abs(price - LAST_BTC_PRICE) >= 8.0:
                        LAST_BTC_MOVE_TS = ts_local_ms
                        LAST_BTC_DIR = 1 if price > LAST_BTC_PRICE else -1
                    LAST_BTC_PRICE = price

        except Exception as e:
            logger.error(f"Binance WS error: {e}. Reconnecting in {retry_delay}s")
            await asyncio.sleep(retry_delay)
            retry_delay = min(retry_delay * 2, 60)


async def coinbase_ws_task() -> None:
    """Coinbase WS — secondary feed for flow confirmation. Does NOT drive btc_price."""
    ...
    while True:
        try:
            async with websockets.connect(url, ping_interval=30, ping_timeout=30) as ws:
                ...
                logger.info("Coinbase WS connected (Secondary Flow Feed).")
                retry_delay = 1
                while True:
                    data = json.loads(await ws.recv())
                    ...
                    STATE.cb_price = price
                    STATE.cb_ts_ms = ts_local
...
async def market_clock_task(end_time: datetime) -> None:
    ...
    while True:
        ...
        if last_ws != ws:
            ...
            UNWIND_CTRLS["UP"].reset()
            UNWIND_CTRLS["DOWN"].reset()
            ...
            try:
                ...
                cur_realized = float(POS_UP.realized_pnl + POS_DOWN.realized_pnl)
...
def get_best_oracle_price() -> tuple:
    ...
    if STATE.btc_ts_ms > 0 and (now - STATE.btc_ts_ms) < BTC_FEED_MAX_AGE_MS * 2:
        if BASIS._init and BASIS.is_stable: return BASIS.corrected_price(STATE.btc_price), "binance_corrected", (now - STATE.btc_ts_ms)
        return STATE.btc_price, "binance", (now - STATE.btc_ts_ms)
...
async def brain_loop(eq: asyncio.Queue) -> None:
    ...
    while True:
        ...
        maint = EXEC_FSM.maybe_tick(_now)
        if maint:
            for _m in maint:
                await eq.put(_m)
        ...
        if (np.isfinite(oracle_px) and not np.isnan(STATE.open_price) and STATE.sec_remaining > 0):
            ...
            LATEST_DEBUG["mom_regime"] = mom_regime
        ...
        if pos.total > 0:
            ...
            for _pm_side, _pm_pos, _pm_tid, _pm_snap, _pm_entry in [("UP", POS_UP, UP_TOKEN_ID, _u_snap, pos.avg_cost_up), ("DOWN", POS_DOWN, DOWN_TOKEN_ID, _d_snap, pos.avg_cost_dn)]:
                ...
                # ── Unwind Controller state update (per-side) ──
                _leg_unwind = UNWIND_CTRLS[_pm_side]
                _unwind_state = _leg_unwind.update(
                    now_ms=now_ms_val,
                    inventory=float(_pm_pos.inventory),
                    urgency=float(_pm_dec.exit_urgency),
                    profit_per_share=float(_pm_dec.profit),
                    ev_sell=float(_pm_dec.sell_ev),
                    ev_hold=float(_pm_dec.hold_ev),
                    spread=_pm_spread,
                    best_bid=_pm_bid,
                    time_remaining=float(STATE.sec_remaining),
                    z=float(z_surv),
                    regime=_regime_lbl,
                    p_side=p_surv if _pm_side == "UP" else (1.0 - p_surv),
                    side=_pm_side,
                    flow_centered=float(LATEST_DEBUG.get("flow_centered", 0.0) or 0.0),
                    best_ask=float(_pm_snap.best_ask) if _pm_snap else 1.0,
                )
                LATEST_DEBUG[f"unwind_state_{_pm_side}"] = _unwind_state
                LATEST_DEBUG[f"book_sane_{_pm_side}"] = _leg_unwind._book_sane
                LATEST_DEBUG[f"thesis_broken_{_pm_side}"] = _leg_unwind._thesis_broken
                LATEST_DEBUG[f"thesis_raw_{_pm_side}"] = _leg_unwind._thesis_broken_raw
                LATEST_DEBUG[f"thesis_streak_{_pm_side}"] = _leg_unwind._thesis_broken_streak

                # Dust suppression: skip all actions if inventory is dust
                if _leg_unwind.suppress_actions(float(_pm_pos.inventory)):
                    continue
                ...
                if _pm_dec.action == "SELL" and not SIMULATION_MODE and ((not EXEC_LOCKED) or _is_stop_loss):
                    _spine = SPINE_UP if _pm_side == "UP" else SPINE_DN
                    _opp_side = "DOWN" if _pm_side == "UP" else "UP"
                    _opp_snap = _d_snap if _pm_side == "UP" else _u_snap
                    _opp_tid = DOWN_TOKEN_ID if _pm_side == "UP" else UP_TOKEN_ID

                    features = Features(
                        now_ms=now_ms_val,
                        price_binance=float(STATE.btc_price or 0.0),
                        price_coinbase=float(STATE.cb_price or 0.0),
                        price_rtds=float(RTDS.price or 0.0),
                        rtds_age_ms=int(ms_now() - RTDS.recv_ts_ms) if RTDS.recv_ts_ms > 0 else 999999,
                        T_sec=float(STATE.sec_remaining),
                        bid=float(_pm_snap.best_bid),
                        ask=float(_pm_snap.best_ask),
                        spread=float(_pm_snap.best_ask - _pm_snap.best_bid),
                        depth=float(_pm_snap.bid_size + _pm_snap.ask_size),
                        bid_depth_1=float(_pm_snap.bid_size),
                        ask_depth_1=float(_pm_snap.ask_size),
                        quote_age_ms=float(LATEST_DEBUG.get("toxic_quote_age_ms", 999)),
                        quote_updates_per_s=float(LATEST_DEBUG.get("toxic_updates_1s", 0)),
                        best_price_changed=True,
                        z=float(z_surv),
                        p_cone=float(p_surv),
                        sigma_1m=float(STATE.sigma_1m),
                        regime=str(_regime_lbl),
                        p_update=float(LATEST_DEBUG.get("p_update", 0.0) or 0.0),
                        oracle_gamma=float(LATEST_DEBUG.get("oracle_gamma", 0.0) or 0.0),
                        oracle_posture=str(LATEST_DEBUG.get("oracle_posture", "MAKER") or "MAKER"),
                        flow=float(LATEST_DEBUG.get("flow_centered", 0.0) or 0.0),
                        flow_accel=float(LATEST_DEBUG.get("flow_accel", 0.0) or 0.0),
                        side=_pm_side,
                        token_id=_pm_tid,
                        inventory=float(_pm_pos.inventory),
                        available_qty=float(max(0.0, float(_pm_pos.inventory) - IN_FLIGHT_QTY.get(_pm_side, 0.0))),
                        in_flight=float(IN_FLIGHT_QTY.get(_pm_side, 0.0)),
                        thesis_broken=bool(_leg_unwind._thesis_broken),
                        book_sane=bool(_leg_unwind._book_sane),
                        unwind_state=str(_unwind_state),
                        sis_urgency=float(_pm_dec.exit_urgency or 0.0),
                        sis_frac=float(_pm_dec.exit_frac or 0.0),
                        sis_offset=float(_pm_dec.exit_offset or 0.0),
                        exit_reason=str(_pm_dec.reason or ""),
                        opp_best_ask=float(_opp_snap.best_ask) if _opp_snap else 1.0,
                        opp_token_id=str(_opp_tid),
                    )
                    ...
                    if decision and not SIMULATION_MODE and not EXEC_LOCKED:
                        ...
                        if req:
                            ...
                            if decision.action == "SELL":
                                IN_FLIGHT_QTY[decision.side] = IN_FLIGHT_QTY.get(decision.side, 0.0) + float(decision.size)
                                IN_FLIGHT_TS[decision.side] = ms_now()

                            if decision.mode == "synthetic_exit":
                                IN_FLIGHT_QTY[decision.side] = IN_FLIGHT_QTY.get(decision.side, 0.0) + float(decision.size)
                                IN_FLIGHT_TS[decision.side] = ms_now()
                                _exit_side = decision.debug.get("exit_side")
                                if _exit_side in ("UP", "DOWN"):
                                    IN_FLIGHT_QTY[_exit_side] = IN_FLIGHT_QTY.get(_exit_side, 0.0) + float(decision.size)
                                    IN_FLIGHT_TS[_exit_side] = ms_now()

                            LATEST_DEBUG["spine_reason"] = decision.reason
                            LATEST_DEBUG["spine_mode"] = decision.mode
                            LATEST_DEBUG["spine_action"] = decision.action
            ...
            _flow_against = ((pos.dominant_side == "UP" and flow_now < -0.70) or (pos.dominant_side == "DOWN" and flow_now > 0.70)) and pos.total > 0
            if _flow_against and now_ms_val - FLOW_REVERSAL_LAST_TS > FLOW_REVERSAL_COOLDOWN_MS:
                FLOW_REVERSAL_LAST_TS = now_ms_val
            ...
            _dom_side = pos.dominant_side
            _dom_unwind = UNWIND_CTRLS[_dom_side] if _dom_side else None

            if _dom_unwind and _dom_unwind.allow_survival():
                ...
                if action in ("EXIT", "TRIM", "HEDGE") and not SIMULATION_MODE and sv.get("token_id") and sv.get("size", 0) > 0:
                    logger.info(f"SURVIVAL {action}: {sv['reason']} size={sv['size']} | unwind={_dom_unwind.state}")
                ...
                LATEST_DEBUG["reason"] = f"SURV_BLOCKED_unwind={_dom_unwind.state if _dom_unwind else 'NONE'}"
            brain_loop._last_survival_ms = now_ms_val
            continue
        ...
        _cb_has_feed = not np.isnan(STATE.cb_price) and STATE.cb_price > 0
        _cb_feed_price = STATE.cb_price if _cb_has_feed else STATE.btc_price
        if (not np.isnan(_cb_feed_price) and _cb_feed_price > 0 and not np.isnan(STATE.open_price) and STATE.open_price > 0 and STATE.sec_remaining > 0 and STATE.sigma_1m > 0):
            _cb_z = CB_LEAD.compute_cb_z(_cb_feed_price, STATE.open_price, STATE.sec_remaining, STATE.sigma_1m)
        ...
        # ── ORACLE LEAD (RTDS Update Predictor) ──
        global _FLOW_EMA
        _ol_flow_centered = float(LATEST_DEBUG.get("flow_centered", 0.0) or 0.0)
        _FLOW_EMA = 0.9 * _FLOW_EMA + 0.1 * _ol_flow_centered
        _ol_flow_accel = _ol_flow_centered - _FLOW_EMA

        _ol_info = ORACLE_LEAD.update(
            p_bin=float(STATE.btc_price),
            p_rtds=float(RTDS.price),
            rtds_age_ms=int(RTDS.age_ms()),
            sigma_1m=float(STATE.sigma_1m),
            flow_accel=float(_ol_flow_accel),
            regime=str(REGIME.label),
        )
        LATEST_DEBUG.update(_ol_info)
        LATEST_DEBUG["flow_ema"] = round(_FLOW_EMA, 4)
        # ──────────────────────────────────────────

        # ── COINBASE FLOW CONFIRMATION ──
        _cb_confirms = False
        _cb_diverges = False
        if (not np.isnan(STATE.cb_price) and STATE.cb_price > 0
                and not np.isnan(STATE.btc_price) and STATE.btc_price > 0
                and np.isfinite(RTDS.price) and RTDS.price > 0
                and (ms_now() - STATE.cb_ts_ms) < 3000):
            _bin_dir = 1 if STATE.btc_price > RTDS.price else (-1 if STATE.btc_price < RTDS.price else 0)
            _cb_dir  = 1 if STATE.cb_price > RTDS.price else (-1 if STATE.cb_price < RTDS.price else 0)
            if _bin_dir != 0:
                _cb_confirms = (_cb_dir == _bin_dir)
                _cb_diverges = (_cb_dir != 0 and _cb_dir != _bin_dir)

        _p_upd = float(LATEST_DEBUG.get("p_update", 0.0) or 0.0)
        if _cb_confirms and _p_upd > 0.0:
            _p_upd = min(1.0, _p_upd * 1.15)   # +15% boost when Coinbase confirms
            LATEST_DEBUG["p_update"] = round(_p_upd, 4)
            LATEST_DEBUG["p_update_cb_boost"] = True
        elif _cb_diverges and _p_upd > 0.0:
            _p_upd = _p_upd * 0.70              # -30% penalty when Coinbase diverges
            LATEST_DEBUG["p_update"] = round(_p_upd, 4)
            LATEST_DEBUG["p_update_cb_boost"] = False
        LATEST_DEBUG["cb_confirms"] = _cb_confirms
        LATEST_DEBUG["cb_diverges"] = _cb_diverges
        # ──────────────────────────────────────────

        # ── ORACLE GAMMA (Execution Posture) ──
        _og_info = {"oracle_gamma": 0.0, "oracle_dt_s": 0.0, "dp_cone": 0.0}
        _og_posture = "MAKER"
        if (not np.isnan(STATE.btc_price) and STATE.btc_price > 0
                and np.isfinite(RTDS.price) and RTDS.price > 0
                and STATE.sec_remaining > 0 and STATE.sigma_1m > 0):
            _og_delta = float(STATE.btc_price) - float(RTDS.price)
            _og_snap_cap = 2.0 * float(RTDS.price) * max(0.00035, float(STATE.sigma_1m))
            _og_pred_rtds = float(RTDS.price) + clamp(_og_delta, -_og_snap_cap, _og_snap_cap)

            _rp_og = getattr(brain_loop, '_last_rp', None)
            _og_cal_alpha = _rp_og.calibration_alpha if _rp_og else 1.0
            _og_p_pred, _ = cone_p_and_z(
                _og_pred_rtds, STATE.open_price, STATE.sec_remaining,
                STATE.sigma_1m, calibration_alpha=_og_cal_alpha,
            )

            _og_info = ORACLE_GAMMA.compute(
                p_update=float(LATEST_DEBUG.get("p_update", 0.0) or 0.0),
                p_bin=float(STATE.btc_price),
                p_rtds=float(RTDS.price),
                sigma_1m=float(STATE.sigma_1m),
                p_cone_now=float(LATEST_DEBUG.get("p_cone", 0.5) or 0.5),
                p_cone_pred=float(_og_p_pred),
            )
            _og_posture = ORACLE_GAMMA.posture(_og_info["oracle_gamma"])

        LATEST_DEBUG.update(_og_info)
        LATEST_DEBUG["oracle_posture"] = _og_posture
        # ──────────────────────────────────────────

        # ── LIQUIDITY VACUUM DETECTOR ──
        if not hasattr(brain_loop, '_last_up_bid'):
            brain_loop._last_up_bid = 0.0
            brain_loop._last_dn_bid = 0.0

        vac = {"vacuum": False, "vacuum_score": 0}
        if up_snap and dn_snap:
            _best_changed_up = up_snap.best_bid != brain_loop._last_up_bid
            brain_loop._last_up_bid = up_snap.best_bid
            _best_changed_dn = dn_snap.best_bid != brain_loop._last_dn_bid
            brain_loop._last_dn_bid = dn_snap.best_bid

            vac_up = VACUUM_UP.update(
                now_ms=now_ms_val, best_bid=up_snap.best_bid, best_ask=up_snap.best_ask,
                bid_depth_1=up_snap.bid_size, ask_depth_1=up_snap.ask_size,
                best_price_changed=_best_changed_up,
            )
            vac_dn = VACUUM_DOWN.update(
                now_ms=now_ms_val, best_bid=dn_snap.best_bid, best_ask=dn_snap.best_ask,
                bid_depth_1=dn_snap.bid_size, ask_depth_1=dn_snap.ask_size,
                best_price_changed=_best_changed_dn,
            )
            # Use the vacuum signal for whichever side has higher score
            vac = vac_up if _og_posture != "MAKER" and vac_up.get("vacuum_score", 0) >= vac_dn.get("vacuum_score", 0) else vac_dn

        LATEST_DEBUG.update({
            "vacuum": vac.get("vacuum", False),
            "vacuum_score": vac.get("vacuum_score", 0),
            "vacuum_churn": vac.get("churn_1s", 0.0),
            "vacuum_depth_collapse": vac.get("depth_collapse", False),
        })

        # ── DEFENSIVE TRAP DETECTOR ──
        _is_toxic_liquidity = False
        _quote_age = 999
        _recent_updates = 0
        if up_snap and dn_snap:
            for _trap_side, _trap_snap, _trap_tid in [("UP", up_snap, UP_TOKEN_ID), ("DOWN", dn_snap, DOWN_TOKEN_ID)]:
                _target_ask_key = (_trap_tid, _trap_snap.best_ask, _trap_snap.ask_size)
                _qa = now_ms_val - QUOTE_FIRST_SEEN.get(_target_ask_key, now_ms_val)
                _updates = PRICE_UPDATE_TIMES.get(_trap_tid, deque())
                _ru = sum(1 for t in _updates if now_ms_val - t < 1000)
                if _qa < 150 and _ru > 8:
                    _is_toxic_liquidity = True
                    _quote_age = _qa
                    _recent_updates = _ru
                    break

        LATEST_DEBUG["toxic_liquidity"] = _is_toxic_liquidity
        if _is_toxic_liquidity:
            LATEST_DEBUG["toxic_quote_age_ms"] = _quote_age
            LATEST_DEBUG["toxic_updates_1s"] = _recent_updates
        # ──────────────────────────────────────────
        ...
        # ── MASTER CALIBRATION TELEMETRY ──
        _mc_side = str(debug.get("side") or "UP")
        _mc_penalty = get_master_calib_penalty(_mc_side, STATE.sec_remaining, _sig_spread,
                                                _sig_lag_p50, _sig_sigma_ratio)
        _mc_key = bucket_key(_mc_side, t_bucket(STATE.sec_remaining), spread_bucket(_sig_spread),
                             lag_bucket(_sig_lag_p50), sigma_bucket(_sig_sigma_ratio))
        _mc_entry = CALIBRATION_MAP.get(_mc_key, {})
        LATEST_DEBUG["mc_penalty"] = round(_mc_penalty, 4)
        LATEST_DEBUG["mc_bucket"] = _mc_key
        LATEST_DEBUG["mc_wr"] = round(_mc_entry.get("wr", 0.0), 3)
        LATEST_DEBUG["mc_fills"] = _mc_entry.get("fills", 0)

        ATTR.log_tick(now_ms_val, {
            "p_update": LATEST_DEBUG.get("p_update"),
            "oracle_gamma": LATEST_DEBUG.get("oracle_gamma"),
            "oracle_posture": LATEST_DEBUG.get("oracle_posture"),
            "vacuum": LATEST_DEBUG.get("vacuum"),
            "toxic": LATEST_DEBUG.get("toxic_liquidity"),
            "stale": LATEST_DEBUG.get("stale_liquidity"),
            "z": LATEST_DEBUG.get("z"),
            "p_cone": LATEST_DEBUG.get("p_cone"),
            "flow": LATEST_DEBUG.get("flow_centered"),
            "regime": LATEST_DEBUG.get("regime"),
            "side": str(debug.get("side", "")),
            "bid": round(eff_up_bid, 4),
            "ask": round(eff_up_ask, 4),
            "spread": round(max(up_sp, dn_sp), 4),
        }, decision=None)
        ...
        # ── Re-entry guard: don't add to a side that already has an active lifecycle ──
        _entry_ctrl = UNWIND_CTRLS.get(side)
        if _entry_ctrl and _entry_ctrl.state != "FLAT":
            LATEST_DEBUG["reason"] = f"REENTRY_BLOCKED_{side}_unwind={_entry_ctrl.state}"
            continue
        ...
        MAX_GROSS_SHARES, MAX_NET_SHARES = int(os.getenv("MAX_GROSS_SHARES", "150")), int(os.getenv("MAX_NET_SHARES", "100"))
        ...
        _net_after = abs(portfolio_net_shares() + (capped_size if side == "UP" else -capped_size))
        if _net_after > MAX_NET_SHARES:
            _net_room = int(MAX_NET_SHARES - abs(portfolio_net_shares()))
            if _net_room <= 0:
                capped_size = 0
            else:
                capped_size = min(capped_size, _net_room)
        ...
        # ── FUSED EXECUTION POLICY (Oracle Lead + Vacuum + Trap) ──
        _p_update = float(LATEST_DEBUG.get("p_update", 0.0) or 0.0)
        _vacuum = LATEST_DEBUG.get("vacuum", False)
        _pm_spread = max(up_sp, dn_sp)

        _spread_tier_exec = debug.get("spread_tier", "maker_only")

        # 1. Base Posture from RTDS Predictor
        if _p_update >= 0.70:
            _spread_tier_exec = "taker_allowed" if _pm_spread <= 0.02 else "maker_preferred"
        elif _p_update >= 0.40:
            _spread_tier_exec = "maker_preferred"

        # 2. Oracle Gamma Override (existing logic, kept as boost)
        if _og_posture == "CROSS" and _spread_tier_exec != "taker_allowed":
            _spread_tier_exec = "taker_allowed"

        # 3. Offensive Upgrade: Liquidity Vacuum
        if _vacuum and _p_update > 0.50:
            if _spread_tier_exec == "maker_only":
                _spread_tier_exec = "maker_preferred"
            elif _spread_tier_exec == "maker_preferred":
                _spread_tier_exec = "taker_allowed"
                _mode = "vacuum_cross"

        # 4. Offensive Upgrade: Stale Quote Snipe
        _stale_liquidity = False
        if _spread_tier_exec != "taker_allowed" and _p_update > 0.55:
            _snipe_snap = up_snap if exec_token == UP_TOKEN_ID else dn_snap
            _snipe_tid = UP_TOKEN_ID if exec_token == UP_TOKEN_ID else DOWN_TOKEN_ID
            _snipe_ask_key = (_snipe_tid, _snipe_snap.best_ask, _snipe_snap.ask_size)
            _snipe_age = now_ms_val - QUOTE_FIRST_SEEN.get(_snipe_ask_key, now_ms_val)
            _snipe_depth = float(_snipe_snap.bid_size) + float(_snipe_snap.ask_size)
            _snipe_spread = float(_snipe_snap.best_ask - _snipe_snap.best_bid)
            if _snipe_age > 600 and _snipe_spread < 0.03 and _snipe_depth > 50:
                _stale_liquidity = True
                _spread_tier_exec = "taker_allowed"
                _mode = "stale_quote_snipe"
                LATEST_DEBUG["stale_quote_age_ms"] = _snipe_age
        LATEST_DEBUG["stale_liquidity"] = _stale_liquidity

        # 5. Defensive Downgrade: Toxic Liquidity Trap
        if _spread_tier_exec == "taker_allowed" and _is_toxic_liquidity:
            logger.info(f"BLOCKED_TOXIC_LIQUIDITY: Downgrading CROSS to MAKER_AGGR "
                        f"(Updates: {_recent_updates}/s, Age: {_quote_age}ms)")
            _spread_tier_exec = "maker_preferred"
            _mode = "trap_avoided"

        debug["spread_tier"] = _spread_tier_exec
        debug["spread_tier_reason"] = (
            "toxic_trap_block" if _is_toxic_liquidity and _spread_tier_exec == "maker_preferred"
            else "stale_quote_snipe" if _stale_liquidity and _spread_tier_exec == "taker_allowed"
            else "vacuum_cross" if _vacuum and _spread_tier_exec == "taker_allowed"
            else "oracle_gamma_cross" if _og_posture == "CROSS"
            else "p_update_base"
        )
        LATEST_DEBUG["posture_after_vacuum"] = _spread_tier_exec
        # ──────────────────────────────────────────
...
def _on_rtds_price(price: float, ts_ms: int) -> None:
    ...
    _basis_ref = STATE.cb_price if (STATE.cb_ts_ms > 0 and not np.isnan(STATE.cb_price)
                                     and (ms_now() - STATE.cb_ts_ms) < 5000) else STATE.btc_price
    if not np.isnan(_basis_ref) and _basis_ref > 0:
        BASIS.update(price, _basis_ref)

    # ── RTDS ground truth label for predictor harness ──
    if not np.isnan(RTDS.prev_price) and abs(price - RTDS.prev_price) > 0.01:
        LATEST_DEBUG["rtds_changed"] = True
        LATEST_DEBUG["rtds_change_ms"] = ms_now()
        LATEST_DEBUG["rtds_p_update_at_change"] = ORACLE_LEAD.p_update
